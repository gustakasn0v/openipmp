...
public:
	...
	//-------------------------------------------------------------------------
	static HRESULT compareTime(
		CComBSTR bstrDateTime1, 
		CComBSTR bstrDateTime2, 
		double	*ptrdCompareResult);

	//-------------------------------------------------------------------------
	//--FUNCTION ReportError
	//-------------------------------------------------------------------------
	//--This function is used for reporting parsing errors which occur during the
	//  loading of xml into a IXMLDOMDocument2Ptr object.
	//--Parameters:  
	//     wostream &wout 							-> a wide output stream to which the error report will be output to.
	//     IXMLDOMParseError *pXMLError 	-> contains the error info.
	//-------------------------------------------------------------------------
	static HRESULT ReportError(CComPtr<IXMLDOMParseError> &rspXMLError, CComBSTR &bstrError);
	...
	//--string, a BSTR
	//-------------------------------------------------------------------------
	static HRESULT appendString(BSTR &rbstrSrc, CComBSTR &rbstrToAppend);
	...
	//-------------------------------------------------------------------------
	static HRESULT loadXML(BSTR &rbstrXML, CComPtr<IXMLDOMDocument> &rspDoc, VARIANT_BOOL &rbSuccess);
	...
	//  the load operation in the parameter rbSuccess
	//-------------------------------------------------------------------------
	static HRESULT load(CComBSTR &rbstrFileName, CComPtr<IXMLDOMDocument> &rspDoc, VARIANT_BOOL &rbSuccess,
	...
	//-------------------------------------------------------------------------
	static void writeLog(CComBSTR& rbstrLog);
	...
	//-------------------------------------------------------------------------
	static int compareBSTR(CComBSTR bstrString1, CComBSTR bstrString2);

protected:
	static double compareTimeHelper(tm &tm1, tm &tm2);
	static TCHAR* substring(TCHAR *tString, int nStartIdx, int nCount);
	...
	//--Example: 2002-10-17T12:57:23 (Oct 10th, 2002, 12:57 with 23 seconds pm)
	//-------------------------------------------------------------------------
	static HRESULT convertDateTime(CComBSTR bstrISODateTime, tm &tmOutTime);

public:
	static bool s_bLog;
	static TCHAR* s_tLogFileName;
