// RELStringArray.cpp: implementation of the CRELStringArray class.
//
//////////////////////////////////////////////////////////////////////
...
//------------------------------------------------------------------------------------------
CRELStringArray::CRELStringArray()
{
	m_bstrBuffer = NULL;
	m_nCount = 0;
	m_nBufferLength = 0;
	m_nIncrementLength = 5;
}
...
//------------------------------------------------------------------------------------------
CComBSTR CRELStringArray::item(int nPos)
{
	CComBSTR bstrItem(NULL);
	if (nPos < m_nCount && m_bstrBuffer != NULL)
	{
		bstrItem = m_bstrBuffer[nPos];
	}
...
//------------------------------------------------------------------------------------------
HRESULT CRELStringArray::addItem(CComBSTR bstrItem)
{
	if (m_nBufferLength == m_nCount)
	{
		m_nBufferLength += m_nIncrementLength;
		CComBSTR *bstrTempBuf = new CComBSTR[m_nBufferLength];
		if (bstrTempBuf == NULL)
		{
			// Can't do much.  
			return E_FAIL;
		...
		{
			for (int nIdx=0; nIdx<m_nCount; nIdx++)
			{
				bstrTempBuf[nIdx] = m_bstrBuffer[nIdx];
			}
			delete [] m_bstrBuffer;
		}
		m_bstrBuffer = bstrTempBuf;
	}

	m_bstrBuffer[m_nCount] = bstrItem;
...
//------------------------------------------------------------------------------------------
void CRELStringArray::clearItems()
{
	if (m_nCount > 0 && m_bstrBuffer != NULL)
	{
		delete [] m_bstrBuffer;
	}
	m_nCount = 0;
