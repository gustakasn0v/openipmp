...
#ifdef RELEXLIMITSERVICE_EXPORTS
#define RELEXLIMITSERVICE_API extern "C" __declspec(dllexport)
#else
#define RELEXLIMITSERVICE_API extern "C" __declspec(dllimport)
#endif

class CRELExLimitService {
public:
	CRELExLimitService(void);
	...
	//------------------------------------------------------------------------------------------
	HRESULT initialize(TCHAR *tszConditionId, int nMaximumCount);
	...
	//------------------------------------------------------------------------------------------
	HRESULT update(TCHAR *tszConditionId, int nExerciseCount);

	static CRELExLimitService s_relExLimitService;
...
protected:
	...
	//--The function returns the remaining exercise count for the given condition
	//------------------------------------------------------------------------------------------
	int getRemainingCount(TCHAR *tszConditionId);

private:
	TCHAR *m_tszStateFileName;
};
