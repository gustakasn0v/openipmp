...
public class osmsDvDoiTitleManagerBase implements Serializable
{
	private String m_tablename="DOI_TITLE";
	...
	private String m_sqlResultSet=
		(
			(new StringBuffer())
			.append(" DOI_TITLE.TITLE_ID")
			.append(", ").append(" DOI_TITLE.SEQUENCE")
			.append(", ").append(" DOI_TITLE.TITLE_TEXT")
			.append(", ").append(" DOI_TITLE.LANGUAGE")
			.append(", ").append(" DOI_TITLE.MAINTENANCE_FLAG")
			.append(", ").append(" DOI_TITLE.LAST_UPDATED_BY")
			.append(", ").append(" DOI_TITLE.LAST_UPDATED_ON")
			.append(", ").append(" DOI_TITLE.CREATED_BY")
			.append(", ").append(" DOI_TITLE.CREATED_ON")
		).toString();

/*
	private String m_sqlResultSet=" DOI_TITLE.TITLE_ID,  DOI_TITLE.SEQUENCE,  DOI_TITLE.TITLE_TEXT,  DOI_TITLE.LANGUAGE,  DOI_TITLE.MAINTENANCE_FLAG,  DOI_TITLE.LAST_UPDATED_BY,  DOI_TITLE.LAST_UPDATED_ON,  DOI_TITLE.CREATED_BY,  DOI_TITLE.CREATED_ON";
*/
	//------------------------------------------------------------------
	// mapResultSet2Bean
	//------------------------------------------------------------------
	protected osmsDvDoiTitleBean mapResultSet2Bean(ResultSet rs)
		throws Exception
	{
			int i=1;

			osmsDvDoiTitleBean bean = new osmsDvDoiTitleBean();
			...
			bean.setTitleText((String)ConvertObject2String(rs.getObject(i++)));
			bean.setLanguage((String)ConvertObject2String(rs.getObject(i++)));
...
/*
		...
		throws Exception
	{
		...
		osmsDvPkControlBean[] bean = managerPkControl.getByTableName("doi_title",conn);
...
*/
	...
	//------------------------------------------------------------------
	protected osmsDvDoiTitleBean[] executeQuery(PreparedStatement statement,Connection conn)
		throws Exception
	{
		ResultSet rs=null;
		osmsDvDoiTitleBean[] ret=null;
		try
		{
			...
			ret = new osmsDvDoiTitleBean[beans.size()];
			for(int i=0;i<beans.size();i++)
				ret[i]=(osmsDvDoiTitleBean)beans.elementAt(i);
	...
	// PROTECTED SELECT
	//------------------------------------------------------------------
	protected osmsDvDoiTitleBean[] get(Connection conn,String postWhere)
		throws Exception
	{
...
*/
	...
	protected osmsDvDoiTitleBean[] getByTitleId(Integer pTitleId,Connection conn,String postWhere)
		throws Exception
	{
		StringBuffer sql = new StringBuffer();
		sql.append("Select  ")
	...
	// SELECT
	//------------------------------------------------------------------
	public osmsDvDoiTitleBean[] get(Connection conn)
		throws Exception
	{
		return get( conn,"");
	}

	public osmsDvDoiTitleBean[] getByTitleId(Integer pTitleId,Connection conn)
		throws Exception
	{
	...
	// UPDATE
	//------------------------------------------------------------------
	public synchronized void update(osmsDvDoiTitleBean bean,Connection conn)
		throws Exception
	{
		...
		if(bean.getTitleText()==null)statement.setNull(i++,java.sql.Types.VARCHAR); else statement.setObject(i++,bean.getTitleText());
		if(bean.getLanguage()==null)statement.setNull(i++,java.sql.Types.VARCHAR); else statement.setObject(i++,bean.getLanguage());
	...
	// INSERT
	//------------------------------------------------------------------
	public void insert(osmsDvDoiTitleBean bean,Connection conn)
		throws Exception
	{
