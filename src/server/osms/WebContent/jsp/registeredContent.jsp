...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" BGCOLOR="#FFFFFF">
	...
	<TR>
		...
		<TD WIDTH="390" VALIGN="TOP" background="key.gif"><IMG SRC="spacer.gif" WIDTH="390" HEIGHT="15" ALIGN="BOTTOM" BORDER="0"><BR>
			...
			WIDTH="12" HEIGHT="13" ALIGN="BOTTOM" BORDER="0"><A HREF="UserKeyStore.jsp?date=<%= Global.getTime() %>" CLASS="nav">Keystore</A><IMG
			SRC="spacer.gif" WIDTH="35" HEIGHT="15" ALIGN="BOTTOM" BORDER="0"><IMG SRC="arrow_on.gif"
			WIDTH="12" HEIGHT="13" ALIGN="BOTTOM" BORDER="0"><FONT CLASS="nav_on">Content<IMG
			SRC="spacer.gif" WIDTH="35" HEIGHT="15" ALIGN="BOTTOM" BORDER="0"><IMG SRC="arrow_off.gif"
			WIDTH="12" HEIGHT="13" ALIGN="BOTTOM" BORDER="0"></FONT><A HREF="ViewLicenses.jsp?date=<%= Global.getTime() %>" CLASS="nav">Licenses</A></TD>
		...
		<TD VALIGN="TOP" COLSPAN="3" BGCOLOR="#FFFFFF">
			<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="686">
				...
				<TR>
					...
					<TD WIDTH="655"><IMG SRC="registered_content.gif" WIDTH="268" HEIGHT="23" ALIGN="BOTTOM" BORDER="0"><BR>
				...
				<TR>
					...
					<TD WIDTH="655" VALIGN="TOP"><BR>
						<FONT CLASS="error"><%=(request.getParameter("errorMessage")==null?"":request.getParameter("errorMessage"))%></FONT>
						<%com.mutable.osms.server.db.osmsDatabase db=com.mutable.osms.server.app.osmsGlobalObjectManager.getInstance().getDatabase();%>
						<%com.mutable.osms.server.bo.osmsContent[] contents= db.getContentByRegisteredBy(user.getUserId());%>
						<%for (int i=0;contents!=null && i<contents.length; i++) {%>
						<%com.mutable.osms.server.bo.osmsDoi ContentDoi = db.getDoiByDoiId( db.getContentDoiByContentId(contents[i].getContentId()).getDoiId());%>
						<A HREF="licensedUsers.jsp?date=<%= Global.getTime() %>&contentId=<%= contents[i].getContentId().toString()%>&primaryTitle=<%=contents[i].getPrimaryTitle() %>&doiIdentifier=<%=ContentDoi.getDoiIdentifier()%>" CLASS="nav_on"><%=contents[i].getPrimaryTitle()%> (<%=ContentDoi.getDoiIdentifier()%>)</A><BR>
						<%com.mutable.osms.server.bo.osmsDigitalItem[] digitalItems= db.getDigitalItemByContentId(contents[i].getContentId());%>
						<%for (int j=0;digitalItems!=null && j<digitalItems.length; j++) {%>
						<IMG SRC="spacer.gif" WIDTH="8" HEIGHT="1" VALIGN="TOP" BORDER="0"><FONT CLASS="bodycopyitalic">
						<%com.mutable.osms.server.bo.osmsDoi doi= db.getDoiByDoiId(digitalItems[j].getDoiId());%>
						<%= doi.getDoiIdentifier()%> - Packaged on: <%=DateTool.convertToString(digitalItems[j].getCreatedOn())%></FONT><BR>
						<% } %>
						<% } %>
					</TD>
