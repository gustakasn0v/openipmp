 ...
 *
 * @version $Id: RSASignSessionBean.java,v 1.2 2006/06/12 15:53:09 danijel Exp $
 */
public class RSASignSessionBean extends BaseSessionBean implements ISignSession {
	...
	/** verifyBytesAndSignedSHA1Digest() - Verify the authenticity of a SHA1 digest that has been previously signed with the
	*                                      CA's Private Key against the original series of bytes.
	*
	*  @param byte[] p_OriginalBytes, byte[] p_SignedDigest
	*  @returns boolean - signatureMatches
	*/
	public boolean verifyBytesAndSignedSHA1Digest(byte[] p_OriginalBytes, byte[] p_SignedDigest) throws RemoteException
	{
 		try
        	{
			if(caCert==null)
				throw new Exception("Signature verification failed because CA Certificate was null");

			debug(">verifyBytesAndSignedSHA1Digest(byte[] p_OriginalBytes, byte[] p_SignedDigest)");
			KeyFactory keyfact = KeyFactory.getInstance("RSA", "BC");

			JDKDigestSignature signature = new JDKDigestSignature.SHA1WithRSAEncryption();
			signature.initVerify(caCert.getPublicKey());
			signature.update(p_OriginalBytes);

 			if(signature.verify(p_SignedDigest))
				return true;
			else
				return false;
		}
 	...
 	/** generateSignedSHA1Digest() - Generates a SHA1 Digest and Signs with the CA's Private Key.
	*
	*  @param byte[] p_OrignalBytes
	*  @returns byte[] - Signed Digest
	*/
	public byte[] generateSignedSHA1Digest(byte p_OriginalBytes[]) throws RemoteException
	{
		try
		{
			if(privateKey==null)
				throw new Exception("Signature processing failed because CA Private Key was null");

			debug(">generateSignedSHA1Digest(byte[] p_Bytes)");
			JDKDigestSignature signature = new JDKDigestSignature.SHA1WithRSAEncryption();
