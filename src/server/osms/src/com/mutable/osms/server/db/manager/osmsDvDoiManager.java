...
public class osmsDvDoiManager extends osmsDvDoiManagerBase implements Serializable
{
	...
	//------------------------------------------------------------------
	// getNextIdentifierNumber
	//------------------------------------------------------------------
	public Integer getNextIdentifierNumber(String identifierType, Connection conn)
		throws Exception
	{
		osmsDvPkControlManagerBase managerPkControl = new osmsDvPkControlManagerBase();
		osmsDvPkControlBean[] bean = managerPkControl.getByTableName(identifierType,conn);
		Integer seq= bean[0].getCurrPkValue();
		bean[0].setCurrPkValue(new Integer(seq.intValue()+1));
		managerPkControl.update(bean[0],conn);
		return new Integer(seq.intValue()+1);
