/**	\file OpenIPMPISMADRMEncManager.h
...
#if !defined (OPENIPMPISMADRMENCMANAGER_H)
#define OPENIPMPISMADRMENCMANAGER_H
...
/*! \brief  ISMA DRM encoding manager using openIPMP infrastructure.

    Creates and stores keys (cipher encryptors), adds licenses for protected
    content.
*/
class OpenIPMPISMADRMEncManager: public IISMADRMEncManager {
public:
  /*! \brief  Constructor.
  ...
  */
  OpenIPMPISMADRMEncManager(IXMLElement* xml, DRMLogger& logger);

  virtual ~OpenIPMPISMADRMEncManager();
...
#endif // !defined (OPENIPMPISMADRMENCMANAGER_H)
