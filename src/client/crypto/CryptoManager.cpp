		       ...
		       char *keymatch)
{
	...
	for (i = 0; i < sk_PKCS12_SAFEBAG_num(bags); i++) {
		if (!OpenIPMP_parse_bag(sk_PKCS12_SAFEBAG_value (bags, i),
			 pass, passlen, pkey, cert, ca, licenses, keyid,
							 keymatch)) return 0;
	}
	return 1;
}
...
void CryptoManager::generateKey(char* p_Alg, int p_LengthInBits)
{
	...
	if (!cipher) {
		//TODO: throw an exception 
		cout << "CryptoManager::generateKey : cannot construct Cipher instance." << endl;
		return;
	}
...
//	ASN1_TYPE_set_octetstring(anotherEntitlement, (unsigned char*)entitlement2, strlen(entitlement2));
	...
	cout << "current number of elements " << sk_num(myEntitlements) << endl;
